...
/**
 * WaveSpinner — animated loading spinner with wave effects (GAIA UI kit).
 *
 * Ported from the GAIA registry component, with the `class-variance-authority`
 * dependency swapped for a plain size map so it drops into this app without a
 * new npm dependency. Behaviour, animations and the reduced-motion fallback
 * (see wave-spinner.css) are unchanged.
 */
...
// Size → CSS custom properties (dot + gap). Replaces the cva size variant.
const SIZE_VARS: Record<SpinnerSize, string> = {
