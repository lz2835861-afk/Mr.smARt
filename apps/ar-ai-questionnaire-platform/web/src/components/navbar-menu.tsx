import { AnimatePresence, motion } from "motion/react"; // PORT: was "framer-motion"; this app ships the `motion` pkg
import * as React from "react";
import { Link } from "react-router-dom"; // PORT: internal hrefs use client-side nav
    ...
    return (
      <li className={cn("list-none", rowSpan === 2 && "row-span-2")}>
        {comingSoon ? (
          ...
          // PORT: internal SPA route → react-router Link (client-side nav)
          <Link to={href} className={sharedClassName}>{inner}</Link>
        ) : (
          <a
            ref={ref}
            href={href}
    ...
    >
      <div className="p-4">
        <ul className={gridLayout}>
          {activeSection.links.map((link, i) => (
            <ListItem
